...
const Section2 = () => {
  ...
  const sectionData: Record<string, SectionData> = {
    ...
    personal: {
      ...
      social: [
        ...
        { name: 'X', url: 'https://x.com/painitedev', icon: 'bi bi-twitter-x text-[#f9f9f9]' },
        { name: 'Facebook', url: 'https://www.facebook.com/painite.2025', icon: Facebook },
        { name: 'TikTok', url: 'https://www.tiktok.com/@painitecode', icon: 'bi bi-tiktok text-[#f9f9f9]' },
  ...
  const SocialIcon = ({ social }: { social: SocialLink }) => {
    ...
    } else if (typeof social.icon === 'string' && social.icon.startsWith('bi ')) {
      // Bootstrap icon
      return (
        <a
          href={social.url}
